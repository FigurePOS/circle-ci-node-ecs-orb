...
if [ -n "$EXPORT_TIMESCALE_CREDENTIALS" ]; then
    TF_VAR_timescale_project_id=$(aws ssm get-parameters --names "timescale_project_id" --region="$AWS_REGION" --query 'Parameters[].Value' --output text)
    echo "export TF_VAR_timescale_project_id=$TF_VAR_timescale_project_id" >> "$BASH_ENV"

    TF_VAR_timescale_public_key=$(aws ssm get-parameters --names "timescale_public_key" --region="$AWS_REGION" --query 'Parameters[].Value' --output text)
    echo "export TF_VAR_timescale_public_key=$TF_VAR_timescale_public_key" >> "$BASH_ENV"

    TF_VAR_timescale_secret_key=$(aws ssm get-parameters --names "timescale_secret_key" --region="$AWS_REGION" --query 'Parameters[].Value' --output text --with-decryption)
